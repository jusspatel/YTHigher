...
async function getData(){
    today = new Date()
    let URL1 = `https:///vid.puffyan.us/api/v1/videos/${randomItems[0]}?fields=title,viewCount,published,videoThumbnails`
    let URL2 = `https:///vid.puffyan.us/api/v1/videos/${randomItems[1]}?fields=title,viewCount,published,videoThumbnails`
...
t = Math.floor(100 * Math.random()) > 5 ? set1[Math.floor(Math.random() * set1.length)] + " " + set2[Math.floor(Math.random() * set2.length)] : setSpecial[Math.floor(Math.random() * setSpecial.length)]
    let urL = `https:///vid.puffyan.us/api/v1/search?q=${t}&fields=videoId&region=${(await (await fetch('http://www.geoplugin.net/json.gp')).json()).geoplugin_countryCode}`;
